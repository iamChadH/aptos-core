...
impl AptosNodeArgs {
    ...
    pub fn run(self) {
        if self.test {
            ...
            // Create a seeded RNG and start the test environment
            let rng = self
                .seed
    ...
    R: ::rand::RngCore + ::rand::CryptoRng,
{
    ...
    let config = if validator_config_path.exists() {
        ...
        let fullnode_network = template.full_node_networks.get_mut(0).unwrap();
...
pub fn setup_environment_and_start_node(
    ...
    utils::start_node_inspection_service(&node_config);
    ...
    let telemetry_runtime = utils::start_telemetry_service(
        &node_config,
        remote_log_rx,
    ...
    let (state_sync_runtimes, mempool_listener, consensus_notifier) =
        state_sync::start_state_sync_and_get_notification_endpoints(
            &node_config,
            storage_service_server_network_handles,
            storage_client_network_senders,
    ...
    let (mempool_client_receiver, api_runtime, index_runtime) =
        utils::bootstrap_api_and_indexer(&node_config, aptos_db, chain_id)?;

    // Create mempool and get the consensus to mempool sender
    let (mempool_runtime, consensus_to_mempool_sender) =
        utils::start_mempool_runtime_and_get_consensus_sender(
            &mut node_config,
            &db_rw,
            mempool_reconfig_subscription,
            &peer_metadata_storage,
